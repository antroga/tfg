...
En primer lugar cargamos (instalamos previamente) las librerias necesarias. 
```{r}
...
Vemos algunos datos caracteristicos del grafo como:
...
Probamos varias representaciones graficas para el grafo para intentar verlo con más claridad. 
Circulo
```{r}
...
```
Las visualización del grafo es poco clara.
...
Viendo la representacion grafica observamos una mayor claridad. 
```{r}
V(g4)
E(g4)
plot(g4)
