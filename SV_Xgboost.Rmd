...
```
Los modelos tienen un desempeño perfecto. Puede deberse a un sobre ajuste de los mismos.


QUIZÁ QUITO DE AQUÍ PABAJO
A continuación los entrenaremos cambiando los parámetros por defecto con kCV, validación cruzada con k = 10.
```{r}
control_class <- trainControl(
  method = "cv",
  number = 10)
```
Para el algoritmo XgbTree se pueden optimizar:
nrounds, max_depth, eta, gamma, colsample_bytree, min_child_weight, subsample

- Etiqueta Y1 (ser nodo aislado)
```{r}
trainY1$Y1 <- as.numeric(trainY1$Y1)
