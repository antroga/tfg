...
Se aplican dos tipos de clustering: jerárquico y con kmeans.
...
```
Cargo el dataframe con las medidas de centralidad, cogiendo solo datos numéricoS, y estandarizo los datos.
```{r}
df <- read.table("./dataframe.txt", header = TRUE, sep = ",")
grafodf <- df[, 3:6 ]
grafodf$Degree <- scale(grafodf$Degree)
grafodf$Centrality <- scale(grafodf$Centrality)
grafodf$Closeness <- scale(grafodf$Closeness)
grafodf$Betweeness <- scale(grafodf$Betweeness)
grafodf
...
ALGORITMO K-MEANS: Matriz de adyacencia

En primer lugar, observamos el valor optimo de k que resulta ser k=4.
```{r}
...
ALGORITMO K-MEANS: Dataframe con medidas de centralidad.
...
CLUSTERING JERÁRQUICO: Matriz de adyacencia.
...
CLUSTERING JERÁRQUICO: Dataframe.
...
plot(matgraf, col=grupos1, main = "Clustering jerárquico k=6")
```
...
Represento gráficamente las comunidades del descubrimiento de comunidades usando la forma de matriz de distancias para poder comparar con estos resultados.
```{r}
set.seed(55)
